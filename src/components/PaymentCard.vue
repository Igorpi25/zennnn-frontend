        ...
        <div class="text-black">
          <h3 class="flex items-center justify-between text-2xl pb-4">
            ...
            <div class="flex-shrink">
              <a href="https://stripe.com/" rel="noreferrer noopener" target="_blank" tabindex="-1">
...
export default {
  ...
  methods: {
    ...
    onCardFormSubmit (e) {
      e.preventDefault()
      if (this.paymentMethod === 'DEFAULT' && this.defaultPaymentMethodId && !this.isNewPaymentMethod) {
        if (this.isInvoice) {
        ...
        const billingValid = this.billingAddress ? this.$refs.form.validate(true) : true
      ...
      if (result.error) {
        ...
        if (this.billingAddress) {
          ...
          if (setBillingAddress.error) {
            this.$logger.info('SetBillingAddress error: ', setBillingAddress.error.message)
          }
        }
    ...
    async createSubscription ({ paymentMethodId, priceId }) {
      try {
        ...
        const response = this.isPromo ? data.createPromoSubscription : data.updatePaymentSubscription
        if (response.error) {
          // The card had an error when trying to attach it to a customer.
          throw response
        }
