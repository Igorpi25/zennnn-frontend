<template>
  <div class="bg-gray-50">
    <div ref="container" class="max-w-screen-md mx-auto px-5 pt-8">
      <!-- PRICE LIST AND PAYMENT TYPE -->
      <template v-if="!addPaymentMethod && !latestInvoiceId">
        <div class="text-black">
          ...
          >
            ...
            <div class="flex items-center w-full">
              <span class="font-bold">{{ oneUsdProduct.price }}</span>
              <span v-if="oneUsdProduct.priceInCurrency" class="pl-1">~</span>
              <span v-if="oneUsdProduct.priceInCurrency" class="pl-1">{{ oneUsdProduct.priceInCurrency }}</span>
              ...
              <div class="relative h-8 flex items-center rounded-md font-semibold text-white bg-yellow-400 border-yellow-400 ml-6 px-2">
                ...
                <div class="absolute left-0 border-transparent -ml-2" style="border-width: 8px 8px 8px 0; border-right-color: inherit;" />
              </div>
            </div>
          </div>
          <template v-else>
            <div
              :class="[currentProduct.mId === currentPriceId ? 'cursor-not-allowed' : 'cursor-pointer', { 'border-blue-500 bg-white': currentPaymentType === 'MONTHLY' }]"
            ...
            >
              <RadioInput
                ...
                :disabled="currentProduct.mId === currentPriceId"
                value="MONTHLY"
                label="MONTHLY"
                hide-details
                class="sm:w-64"
              ...
              <div class="flex flex-wrap sm:flex-no-wrap justify-end sm:justify-start items-center w-full" :class="{ 'opacity-40': currentProduct.mId === currentPriceId }">
              ...
              <div v-if="currentProduct.mId === currentPriceId" class="absolute top-0 -mt-2 bg-green-500 text-white text-xs rounded px-xs">
                {{ $t('payment.currentPlan') }}
              </div>
            </div>
            <div
              :class="[currentProduct.aId === currentPriceId ? 'cursor-not-allowed' : 'cursor-pointer', { 'border-blue-500 bg-white': currentPaymentType === 'ANNUALLY' }]"
            ...
            >
              <RadioInput
                v-model="currentPaymentType"
                :disabled="currentProduct.aId === currentPriceId"
                value="ANNUALLY"
                label="ANNUALLY"
                hide-details
                class="sm:w-64"
              ...
              <div class="flex flex-wrap sm:flex-no-wrap justify-end sm:justify-start items-center w-full" :class="{ 'opacity-40': currentProduct.aId === currentPriceId }">
                ...
                <div class="relative sm:h-8 flex items-center rounded-md font-semibold text-white bg-yellow-400 border-yellow-400 ml-6 px-2">
                  {{ $t('payment.econ') }}
                  <div class="hidden sm:block absolute left-0 border-transparent -ml-2" style="border-width: 8px 8px 8px 0; border-right-color: inherit;" />
                </div>
              </div>
              <div v-if="currentProduct.aId === currentPriceId" class="absolute top-0 -mt-2 bg-green-500 text-white text-xs rounded px-xs">
                {{ $t('payment.currentPlan') }}
              </div>
            </div>
          </template>
          <div class="flex justify-between text-xl sm:text-2xl pt-10 pr-4 mr-xs">
            ...
            <div class="hidden sm:block md:w-64 mx-8" />
            <div v-if="oneUsd" class="flex items-center w-full pl-4 ml-xs">
              <span class="font-bold">{{ oneUsdProduct.price }}</span>
              <span v-if="oneUsdProduct.priceInCurrency" class="pl-1">~</span>
              <span v-if="oneUsdProduct.priceInCurrency" class="pl-1">{{ oneUsdProduct.priceInCurrency }}</span>
            </div>
      ...
      <div class="max-w-screen-md">
        <div
          v-if="defaultPaymentMethodId && !addPaymentMethod"
        ...
        <div class="flex sticky bottom-0 bg-gray-50 border-t border-gray-75 mt-10 -mx-xs py-10">
          <Checkbox
          ...
          >
            {{ addPaymentMethod ? $t('payment.add') : $t('payment.buy') }}
          </Button>
        </div>
      </div>
    </div>
...
<script>
import { RETRY_INVOICE_WITH_NEW_PAYMENT_METHOD, CREATE_PAYMENT_SUBSCRIPTION, CREATE_PROMO_SUBSCRIPTION, CHANGE_PAYMENT_SUBSCRIPTION, ATTACH_PAYMENT_METHOD } from '../graphql/mutations'
...
export default {
  ...
  watch: {
    ...
    visibility (val) {
      if (val) {
        if (this.selectedProduct.id === this.currentProductId) {
          if (this.currentPriceId === this.selectedProduct.mId) {
            this.currentPaymentType = 'ANNUALLY'
          }
          if (this.currentPriceId === this.selectedProduct.aId) {
  ...
  methods: {
    ...
    initStripe () {
      const locale = this.$i18n.locale === 'zh-Hans' || this.$i18n.locale === 'zh-Hant' ? 'zh' : this.$i18n.locale
    ...
    selectPaymentType (type) {
      if (type === 'MONTHLY' && this.currentProduct.mId === this.currentPriceId) return
      if (type === 'ANNUALLY' && this.currentProduct.aId === this.currentPriceId) return
    ...
    showCardError (event) {
      if (event) {
        ...
        const message = event.error && event.error.message ? event.error.message : event.error || event
    ...
    onCardFormSubmit (e) {
      e.preventDefault()
      if (this.paymentMethod === 'DEFAULT' && this.defaultPaymentMethodId && !this.addPaymentMethod) {
        if (this.latestInvoiceId) {
          this.retryInvoiceWithNewPaymentMethod({
            paymentMethodId: this.defaultPaymentMethodId,
            invoiceId: this.latestInvoiceId,
          })
        } else {
          if (this.changePrice) {
            this.changeSubscription(this.selectedPriceId)
          } else {
        ...
        if (this.cardElementComplete && this.$refs.form.validate(true)) {
    ...
    async createPaymentMethod (cardElement, priceId) {
      ...
      if (result.error) {
        ...
        if (this.addPaymentMethod) {
          ...
          if (this.changePrice) {
            this.changeSubscription(priceId)
          } else {
            this.createSubscription({
              paymentMethodId: result.paymentMethod.id,
              priceId,
    ...
    async attachPaymentMethod (paymentMethodId) {
      try {
        ...
        this.$emit('complete', 'attachPaymentMethod')
        setTimeout(() => {
          this.reset()
        }, 250)
      } catch (error) {
    ...
    async createSubscription ({ paymentMethodId, priceId }) {
      try {
        this.loading = true
        const mutation = this.oneUsd ? CREATE_PROMO_SUBSCRIPTION : CREATE_PAYMENT_SUBSCRIPTION
        const variables = this.oneUsd
          ? {
            paymentMethodId,
          }
          : {
        ...
        const response = this.oneUsd ? data.createPromoSubscription : data.createPaymentSubscription
        if (response.error) {
          // The card had an error when trying to attach it to a customer.
          throw response
        }
    ...
    async changeSubscription (priceId) {
      this.loading = true
      try {
        await this.$apollo.mutate({
    ...
    onSubscriptionComplete ({ subscription }) {
      ...
      if (subscription && subscription.status === 'active') {
        ...
        this.$notify({ color: 'info', text: this.$t('payment.operationComplete') })
      }
