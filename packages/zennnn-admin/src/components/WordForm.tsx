...
import { Btn, Progress, Icon, Form, Select, TextField } from '@zennnn/core'
  ...
  setup(props, { emit }) {
    ...
    useRender(() => (
      <>
        ...
        >
          {locales.value.map((item) => (
            <TextField
              ...
              onInput={(v: string) => onInput(item.value, v)}
              controlClass="px-2"
              v-slots={{
                prepend: () => (
                  <img
        ...
        <div class="grid grid-rows-1 sm:grid-cols-2 gap-x-8 gap-y-4 pt-4">
          <Select
            v-model={defaultLocale.value}
            items={locales.value}
            label={t('words.defaultLanguage')}
          />
