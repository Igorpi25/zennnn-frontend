import { h, ref, watch, computed, defineComponent, Prop } from 'vue'
...
function filterFn(
  item: any,
  search: string | null,
  filter: DataTableFilterFunction
) {
...
function searchTableItems(
  ...
  if (headersWithCustomFilters.length) {
    filtered = filtered.filter((item) =>
      headersWithCustomFilters.every(filterFn(item, search, defaultFilter))
    )
  }
  return filtered
}
...
export default defineComponent({
  ...
  setup(props, { slots, emit }) {
    ...
    const borderSpacing = computed(() => {
      return props.rounded ? '0 4px' : props.borderSpacing
    })

    const itemsLength = computed(() => {
      return props.serverItemsLength >= 0
    ...
    const columnSorters = computed((): Record<
      string,
      DataTableCompareFunction
    > => {
      return computedHeaders.value.reduce<
        Record<string, DataTableCompareFunction>
      >((acc, header) => {
    ...
    const headersWithCustomFilters = computed(() =>
      computedHeaders.value.filter((header) => header.filter)
    )

    const headersWithoutCustomFilters = computed(() =>
      computedHeaders.value.filter((header) => !header.filter)
    )
        ...
        if (group) {
          ...
          group.forEach((el) => {
            const group = {
              group: true,
              groupName: el.name,
              groupItemsCount: el.items.length,
            }
    ...
    watch(
      () => internalOptions.value.itemsPerPage,
      (itemsPerPage) => {
    ...
    watch(
      ...
      (sortBy, old) => {
        !deepEqual(sortBy, old) &&
          emit(
            'update:sort-by',
            Array.isArray(props.sortBy) ? sortBy : sortBy[0]
          )
      }
    )
    ...
    watch(
      ...
      (sortDesc, old) => {
        !deepEqual(sortDesc, old) &&
          emit(
            'update:sort-desc',
            Array.isArray(props.sortDesc) ? sortDesc : sortDesc[0]
          )
      }
    )
    ...
    watch(
      () => internalOptions.value.groupBy,
      (groupBy, old) => {
        !deepEqual(groupBy, old) &&
          emit(
            'update:group-by',
            Array.isArray(props.groupBy) ? groupBy : groupBy[0]
          )
      }
    )
    ...
    watch(
      () => internalOptions.value.groupDesc,
      (groupDesc, old) => {
        !deepEqual(groupDesc, old) &&
          emit(
            'update:group-desc',
            Array.isArray(props.groupDesc) ? groupDesc : groupDesc[0]
          )
      }
    )
    ...
    function customSortWithHeaders(
      items: any[],
      sortBy: string[],
      sortDesc: boolean[],
      locale: string
    ...
    function customFilterWithColumns(items: any[], search: string) {
      return searchTableItems(
        items,
        search,
        headersWithCustomFilters.value,
        headersWithoutCustomFilters.value,
        props.customFilter as DataTableFilterFunction
      )
    }
    ...
    function genTHead() {
      ...
      const children = computedHeaders.value.map((header, i) => {
        if (slots[`header-${header.value}`])
          return slots[`header-${header.value}`]!({
            value: header.value,
            header: header,
          })
        return h(
          'th',
          {
            ...
            class: [
              ...
              header.sortable &&
              internalOptions.value.sortBy.includes(header.value)
                ? `active ${
                    internalOptions.value.sortDesc[
                      internalOptions.value.sortBy.findIndex(
                        (k) => k === header.value
                      )
                    ]
                      ? 'desc'
          ...
          [
            ...
            h(
              Icon,
              { size: 24, class: 'data-table-header__icon' },
              { default: () => ziArrowSortTop }
            ),
          ]
        )
      })
      ...
      return computedItems.value.map((item, i) => {
        if (item.group) {
          ...
          const children = computedHeaders.value.map((header) => {
            if (slots[`item-${header.value}`])
              return slots[`item-${header.value}`]!({
  ...
  render() {
    return h(
      ...
      [this.genSlot('top'), this.genTable(), this.genNoDataSlot()]
    )
  },
})
