<template>
  <div
    :class="
      light ? 'bg-light-gray-100 border-b border-light-gray-400' : 'bg-gray-400'
    "
  >
    <div class="container">
      <div :class="[light ? 'h-20' : 'h-14', 'flex']">
        ...
        <div class="flex items-center">
          ...
          <template v-if="$route.name === 'paper' && paperOrgName">
            ...
            <span class="text-lg text-white leading-none">{{
              paperOrgName
            }}</span>
          </template>
        </div>
            ...
            >
              <template v-slot:activator>
                ...
                >
                  ...
                  <div
                    :class="[
                      light ? 'bg-gray-100' : 'bg-gray-300',
                      'w-px h-5 mx-3 sm:mx-5',
                    ]"
                  />
                  <div
                    v-if="currentOrg"
                  ...
                  >
                    ...
                    <div
                      :class="[
                        light ? 'text-gray-200' : 'text-gray-100',
              ...
              >
                <span>{{ item.text }}</span>
                <div
                  v-if="item.value === 'pricing' && profile.account && isOwner"
              ...
              <div class="flex items-center">
                <div class="flex-shrink-0 w-9">
                  ...
                  >
                    ...
                    >
                      <template
                        v-slot:drag="{ internalSrc, isDragOver, loading }"
                      >
                        <div class="w-8 h-8 rounded-full">
                          ...
                          <div v-else :class="['w-full h-full rounded-full']">
                            ...
                            <div
                              v-if="
                                isDragOver || updateOrgImageHovered[item.id]
                              "
                              :class="[
                ...
                <div
                  ...
                  @click.prevent.stop="addFavorites(item.id)"
                >
                  <Icon>
                    {{
    ...
    <Modal v-model="systemMessageDialog" max-width="500">
      <SystemMessageModal
...
export default {
  ...
  computed: {
    isOwner() {
      return this.currentOrg.role === Role.OWNER
    },
    loggedIn() {
    ...
    profileItems() {
      ...
      if (
        this.currentOrg.role === Role.OWNER ||
        this.currentOrg.role === Role.MANAGER
      ) {
        items.splice(1, 0, {
          value: 'requisites',
          text: this.$t('header.requisites'),
        })
      }
      return items
    },
    ...
    username() {
      ...
      return profile.familyName ? `${name} ${profile.familyName}` : name
    },
    productName() {
      if (
  ...
  mounted() {
    this.favorites =
      JSON.parse(localStorage.getItem(this.favoritesKeyStore)) || []
    ...
    profileAction(value) {
      switch (value) {
        case 'orgsList':
          ...
          break
        case 'dictionary':
          return this.goToDictionary()
        case 'requisites':
          return this.goToRequisites()
        case 'pricing':
          return this.goToPricing()
        case 'logout':
