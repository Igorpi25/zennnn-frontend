  ...
  <div class="flex-grow flex flex-col">
    ...
    <main class="container flex-grow max-w-screen-md mx-auto pt-16 pb-32">
        ...
        <div class="flex sm:flex-1 items-center w-full">
          <div
            class="
              ...
              flex-shrink-0
            "
            :class="{ 'bg-gray-100': !profile.picture }"
          >
            <img
          ...
          <div class="flex justify-between">
            <div class="">
              ...
              <div v-if="canChange || canSelect">
                <Modal v-model="changeDialog" max-width="458" eager>
                  ...
                  <PriceSelect
                    ...
                    :current-product-id="
                      profile.account && profile.account.productId
                    "
                    :current-price-id="
                      profile.account && profile.account.priceId
                    "
            ...
            <div v-if="profile.account" class="text-right">
              ...
              <div v-if="profile.account.periodEnd">
                <span v-if="subscriptionStatus !== 'expired'" class="mr-1">{{
                  $t('payment.until')
                }}</span>
                <span>{{
                  $d($parseDate(profile.account.periodEnd * 1000))
                }}</span>
              </div>
            </div>
          </div>
      ...
      >
        ...
        <Btn :to="{ name: 'payment', params: { type: 'promo' } }" class="ml-4">
          {{ $t('payment.proceed') }}
        </Btn>
      </div>
      ...
      <div v-if="paymentMethods.length > 0 || defaultPm">
        ...
        <template v-if="defaultPm">
          ...
          <div class="overflow-x-auto scrolling-touch pb-10">
            <table
              class="w-full border-separate text-gray-200 border-spacing-y-1"
            >
              <tbody>
                <tr class="h-12 bg-white hover:bg-transparent group">
                  ...
                  <td width="156" class="capitalize px-4">
                    {{ defaultPm.card.brand }}
                  </td>
                  <td class="px-4">
                    <div class="flex items-center">
                      <div v-for="g in 3" :key="`g-${g}`" class="flex pr-3">
                        <div
                          v-for="i in 4"
                          :key="`i-${i}`"
                  ...
                  <td class="px-4">
                    {{
                      defaultPm.card.exp_month.toString().padStart(2, '0')
                    }}/{{ defaultPm.card.exp_year }}
                  </td>
                  <td width="56" class="rounded-r-md text-gray-100 px-4"></td>
                </tr>
              </tbody>
            </table>
          </div>
          ...
          >
            <thead>
              <tr class="h-12 text-gray-100">
                <td width="146" class="px-4">
                  {{ $t('payment.invoiceNumber') }}
                </td>
                <td width="156" class="pl-4 pr-2">
                  {{ $t('payment.invoiceDate') }}
                </td>
                <td width="116" class="text-right pl-2 pr-4">
                  {{ $t('payment.invoiceAmount') }}
                </td>
                <td width="146" class="px-4">
                  {{ $t('payment.invoiceStatus') }}
                </td>
                <td width="100%"></td>
...
export default {
  ...
  setup() {
    ...
    const { result: result1, refetch: getProfileRefetch } = useQuery(
      GET_PROFILE,
      null,
    ...
    const { result: result2, refetch: listPaymentMethodsRefetch } = useQuery(
      LIST_PAYMENT_METHODS,
      null,
    ...
    const { result: result3, refetch: listPaymentInvoicesRefetch } = useQuery(
      LIST_PAYMENT_INVOICES,
      null,
      { fetchPolicy: 'no-cache' }
    )
  ...
  computed: {
    ...
    productName() {
      if (
        ...
        (!this.profile.account.subscriptionId ||
          this.profile.account.price === 'Trial' ||
          this.profile.account.price === 'Promo')
      )
        return ''
    ...
    canChange() {
      return (
        this.profile.account &&
        this.profile.account.subscriptionId &&
        (this.profile.account.subscriptionStatus === 'active' ||
          this.profile.account.subscriptionStatus === 'trialing')
      )
    },
    canSelect() {
    ...
    isCanceled() {
      return (
        this.profile.account &&
        this.profile.account.subscriptionId &&
        this.profile.account.cancelAtPeriodEnd &&
        !(
          this.profile.account.price === 'Trial' ||
          this.profile.account.price === 'Promo'
        )
