  ...
  <div class="flex-grow flex flex-col">
    ...
    <main class="container flex-grow max-w-screen-md mx-auto pt-16 pb-32">
      ...
      <div class="text-black flex flex-col sm:flex-row justify-between pt-16">
        <h3 class="text-2xl pb-6 sm:pb-0">{{ $t('payment.selectedPlan') }}:</h3>
        <Select
          ...
          :input-class="
            selectedProduct
              ? 'placeholder-gray-100 text-gray-900 font-semibold text-right'
      ...
      <div v-else class="text-black">
        ...
        <template v-else>
          <div
            ...
            :class="[
              { 'border-blue-500 bg-white': currentPaymentType === 'MONTHLY' },
            ]"
            class="
              h-14
              relative
          ...
          >
            ...
            >
              ...
              <span v-if="selectedProduct.mPriceInCurrency" class="pl-1"
                >~</span
              >
              <span v-if="selectedProduct.mPriceInCurrency" class="pl-1">{{
                selectedProduct.mPriceInCurrency
              }}</span>
          ...
          <div
            ...
            :class="[
              { 'border-blue-500 bg-white': currentPaymentType === 'ANNUALLY' },
            ]"
            class="
              h-14
              relative
          ...
          >
            ...
            >
              ...
              <span v-if="selectedProduct.aPriceInCurrency" class="pl-1"
                >~</span
              >
              <span class="pl-1">{{
                selectedProduct.aPriceInCurrency || selectedProduct.aPrice
              }}</span>
              <span class="whitespace-nowrap pl-1">{{
                $t('payment.inMonth')
              }}</span>
        ...
        <div class="flex justify-between text-xl sm:text-2xl pt-10 pr-4 mr-0.5">
          ...
          >
            <span class="font-bold">{{
              currentPaymentType === 'ANNUALLY'
                ? selectedProduct.aPriceTotal
            ...
            <span
              v-if="
                selectedProduct.mPriceInCurrency ||
                selectedProduct.aPriceInCurrency
              "
              class="pl-1"
              >~</span
            >
      ...
      <ExpandTransition>
        <div v-show="selectedProduct && selectedProduct.name !== 'Premium'">
          ...
          <PaymentCard
            ...
            :billing-address="
              profile.account && !profile.account.hasBillingAddress
            "
            @back="goBack"
            @complete="onComplete"
          />
...
export default {
  ...
  setup() {
    ...
    const { result: result3 } = useQuery(LIST_PAYMENT_METHODS, null, {
  ...
  computed: {
    ...
    paymentType() {
      ...
      if (type === 'promo') {
        return this.profile.account &&
          this.profile.account.canPromo &&
          this.selectedProduct.name === 'Advanced'
          ? 'promo'
          : 'change'
      }
      return type
    ...
    selectedProduct: {
      get() {
        ...
        const routeProduct = this.products.find(
          (el) => el.id === this.$route.query.product
        )
  ...
  methods: {
    selectPaymentType(type) {
      if (
        type === 'MONTHLY' &&
        this.selectedProduct.mId === this.currentPriceId
      )
        return
      if (
        type === 'ANNUALLY' &&
        this.selectedProduct.aId === this.currentPriceId
      )
        return
