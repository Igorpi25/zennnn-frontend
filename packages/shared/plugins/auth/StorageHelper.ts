...
let dataMemory = {} as Record<string, any>
...
export default class StorageHelper {
  ...
  constructor(memory = false) {
    try {
      if (memory) {
        throw new Error('Forced use of memory storage.')
      }
      this.storageWindow = window.localStorage
      this.storageWindow.setItem('test-ls', '1')
      this.storageWindow.removeItem('test-ls')
    } catch (exception) {
