export const CURRENT_LOCALE_STORE_KEY = 'zn.current-locale:'

export const ICON_IMAGE_POSTFIX = '-256x256'

export const PREVIEW_IMAGE_POSTFIX = '-640x640'

export const UPLOAD_FILE_SIZE_MB = 10

export const IMAGE_FILENAME_METADATA = 'x-amz-meta-file_name'

export const BACKEND_VERSION_HEADER_KEY = 'backend-version'

export const LOCALES_LIST = [
  { value: 'en', text: 'English', icon: 'United-Kingdom' },
  { value: 'zh-Hans', text: '简体', icon: 'China' },
  { value: 'zh-Hant', text: '繁体', icon: 'Hong-Kong' },
  { value: 'fr', text: 'Français', icon: 'France' },
  { value: 'ru', text: 'Русский', icon: 'Russia' },
  { value: 'uk', text: 'Український', icon: 'Ukraine' },
]
